...
def goster():
    kelime = kelime_giris.get("1.0", "end-1c") 
    yenikelime = harf_degistir(kelime)
    sonuc_text.delete("1.0", tk.END) 
    sonuc_text.insert(tk.END, "\\\\b(?i)"+yenikelime)  
    panoya_kopyala_button.config(state="normal")

def panoya_kopyala():
    kelime = sonuc_text.get("1.0", "end-1c")  
...
olustur_dugme = tk.Button(root, text="Oluştur", command=goster)
